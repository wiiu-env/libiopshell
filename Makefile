...
#---------------------------------------------------------------------------------
all: lib/libiopshell.a

dist-bin: all
	@tar --exclude=*~ -cjf libiopshell-$(VERSION).tar.bz2 include lib

dist-src:
	@tar --exclude=*~ -cjf libiopshell-src-$(VERSION).tar.bz2 include source Makefile

dist: dist-src dist-bin

install: dist-bin
	mkdir -p $(DESTDIR)$(DEVKITPRO)/wums
	bzip2 -cd libiopshell-$(VERSION).tar.bz2 | tar -xf - -C $(DESTDIR)$(DEVKITPRO)/wums
...
lib/libiopshell.a :$(SOURCES) $(INCLUDES) | lib release
